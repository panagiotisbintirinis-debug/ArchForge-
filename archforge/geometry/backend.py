...
@dataclass(frozen=True)
class GeometryBody:
    """Backend-neutral evaluated body plus explicit fabrication evidence.

    A backend must never imply printability merely because it produced something visible.
    ``watertight`` and ``manifold`` stay None until a capable backend actually checks them.
    """
    entity_id: str
    semantic_kind: str
...
class GeometryBackend(ABC):
    ...
    @abstractmethod
    def evaluate_plan(self, doc, plan: EvaluationPlan) -> GeometryEvaluation: raise NotImplementedError
...
class ContractBackend(GeometryBackend):
    """CI-fast semantic contract gate; deliberately not a solid/mesh generator."""
    name = 'contract'
    ...
    def evaluate_plan(self, doc, plan: EvaluationPlan) -> GeometryEvaluation:
        ...
        for node in plan.geometry_nodes():
            surfaces=surface_catalog(doc,node.entity_id); keys=tuple(s.key for s in surfaces); modifier_ids=[]
            for raw in node.modifiers:
                if not raw.get('enabled',True): continue
                mid=str(raw.get('id','')); target=raw.get('target',{})
                try:
                    surface=resolve_surface(doc,node.entity_id,str(target.get('surface_role','')))
                    if target.get('owner_id')!=node.entity_id: raise ValueError('modifier owner does not match evaluation node')
                    if not surface.deformable: raise ValueError('modifier target is not deformable')
