...
def test_sculpt_moves_with_mechanical_part():
    """MoveEntities must support moving mechanical_part and its attached modifiers."""
    doc = Document()
    ...
    modifier = sculpt_modifier_from_hit(doc, hit, BrushSpec(0.4), 'pull', 0.2)
    doc.add_surface_modifier(modifier)
    ...
    # Base part top is now z=6.0; sculpt pulls by 0.2 -> max z should be 6.2
...
def test_sculpt_moves_with_box():
    ...
    modifier = sculpt_modifier_from_hit(doc, hit, BrushSpec(0.5), 'pull', 0.3)
    doc.add_surface_modifier(modifier)
...
def test_sculpt_moves_multiple_entities():
    """MoveEntities with multiple entities must translate modifiers for each moved entity."""
    doc = Document()
    ...
    assert doc.surface_modifiers[m1.id].target.subregion['world_center'] == [3.0, 2.1, 1.5]
    assert doc.surface_modifiers[m2.id].target.subregion['world_center'] == [3.0, 7.1, 1.5]
