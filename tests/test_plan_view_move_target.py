...
def test_move_tool_acquires_clicked_entity_and_clears_stale_target_on_empty_click():
    hit = object()
    view = _view({hit: "wall-2"})

    assert PlanView._acquire_move_target(view, hit) is True
    assert view.doc.selection == ["wall-2"]
    assert view.controller.target == ("wall-2", None)
    assert view.selectionChangedByView.count == 1

    empty = object()
    assert PlanView._acquire_move_target(view, empty) is False
